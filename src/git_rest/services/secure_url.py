...
class SecureURLGenerator:
    def __init__(self, secret=None):
        self.secret = secret or os.environ.get("GIT_REST_SECRET_KEY", "changeme-super-secret-key")
    ...
    def verify(self, repo_id, file_path, expires, token):
        data = f"{repo_id}:{file_path}:{expires}"
        expected_sig = hmac.new(self.secret.encode(), data.encode(), hashlib.sha256).digest()
