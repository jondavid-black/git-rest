...
def audit_repo_action(action):
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            file_path = kwargs.get('file_path') if 'file_path' in kwargs else (args[1] if len(args) > 1 else None)
            logger.info(f"[AUDIT] user={user} action={action} repo_id={repo_id} file_path={file_path} args={args} kwargs={kwargs}")
