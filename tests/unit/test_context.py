...
def test_configure_environment_sets_log_and_workdir(tmp_path, monkeypatch, caplog):
    ...
    # Check log output
    found_log = any("Logging initialized at DEBUG level." in r.message for r in caplog.records)
    found_workdir = any(f"Workdir set to {tmp_path}" in r.message for r in caplog.records)
    assert found_log
